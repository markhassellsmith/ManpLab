...
public class ImageExportService : IImageExportService
{
    ...
    private async Task AddPngMetadataAsync(BitmapEncoder encoder, FractalMetadata metadata)
    {
        ...
        {
            // Layer 1: Complete JSON blob for programmatic reproduction
            new BitmapPropertySet
            {
                { "/tEXt/ManpLabMetadata", new BitmapTypedValue(jsonMetadata, Windows.Foundation.PropertyType.String) }
            },

            // Layer 2: Software attribution (critical for open source)
            new BitmapPropertySet
            {
                { "/tEXt/Software", new BitmapTypedValue($"ManpLab v{metadata.Version}", Windows.Foundation.PropertyType.String) }
            },
            new BitmapPropertySet
            {
                { "/tEXt/Source", new BitmapTypedValue("https://github.com/markhassellsmith/ManpLab", Windows.Foundation.PropertyType.String) }
            },
            new BitmapPropertySet
            {
                { "/tEXt/Comment", new BitmapTypedValue("Generated by ManpLab - Open source fractal explorer", Windows.Foundation.PropertyType.String) }
            },
            new BitmapPropertySet
            {
                { "/tEXt/Creation Time", new BitmapTypedValue(metadata.RenderDate.ToString("yyyy-MM-dd HH:mm:ss"), Windows.Foundation.PropertyType.String) }
            },

            // Layer 3: Fractal identification
            new BitmapPropertySet
            {
                { "/tEXt/FractalFamily", new BitmapTypedValue(metadata.FractalFamily, Windows.Foundation.PropertyType.String) }
            },
            new BitmapPropertySet
            {
                { "/tEXt/FractalName", new BitmapTypedValue(metadata.VisualizationName ?? metadata.FractalType, Windows.Foundation.PropertyType.String) }
            },
            new BitmapPropertySet
            {
            ...
            // Layer 4: Key mathematical parameters (for metadata browsers)
            new BitmapPropertySet
            {
                { "/tEXt/CenterX", new BitmapTypedValue(metadata.CenterX.ToString("R"), Windows.Foundation.PropertyType.String) }
            },
            new BitmapPropertySet
            {
                { "/tEXt/CenterY", new BitmapTypedValue(metadata.CenterY.ToString("R"), Windows.Foundation.PropertyType.String) }
            },
            new BitmapPropertySet
            {
            ...
            new BitmapPropertySet
            {
                { "/tEXt/IterationMode", new BitmapTypedValue(metadata.IterationMode, Windows.Foundation.PropertyType.String) }
            },

            // Layer 5: Rendering parameters
            new BitmapPropertySet
            {
            ...
            new BitmapPropertySet
            {
                { "/tEXt/Resolution", new BitmapTypedValue($"{metadata.ImageWidth}x{metadata.ImageHeight}", Windows.Foundation.PropertyType.String) }
            },
            new BitmapPropertySet
            {
                { "/tEXt/RenderTime", new BitmapTypedValue($"{metadata.ComputeTimeMs / 1000.0:F3}s", Windows.Foundation.PropertyType.String) }
            }
        };

        // Add Julia parameters if applicable
        if (metadata.JuliaC != null)
        {
            var juliaProperties = new[]
            {
                new BitmapPropertySet
                {
                    { "/tEXt/JuliaCX", new BitmapTypedValue(metadata.JuliaC.Real.ToString("R"), Windows.Foundation.PropertyType.String) }
                },
                new BitmapPropertySet
                {
                    { "/tEXt/JuliaCY", new BitmapTypedValue(metadata.JuliaC.Imaginary.ToString("R"), Windows.Foundation.PropertyType.String) }
                }
            };

            metadataProperties = metadataProperties.Concat(juliaProperties).ToArray();
        }
    ...
    private async Task AddJpegMetadataAsync(BitmapEncoder encoder, FractalMetadata metadata)
    {
        ...
        // Build description with key parameters
        var description = $"{metadata.FractalFamily} - {metadata.VisualizationName ?? metadata.FractalType}";
        if (metadata.IterationMode == "Julia")
        {
            description += " (Julia set)";
        }
        description += $" | Center: ({metadata.CenterX:G6}, {metadata.CenterY:G6}) | Zoom: {metadata.Zoom:G6} | Iterations: {metadata.MaxIterations}";

        var metadataProperties = new BitmapPropertySet
        {
            // Layer 1: Complete JSON blob (EXIF UserComment)
            { "/app1/ifd/exif/{ushort=37510}", new BitmapTypedValue(jsonMetadata, Windows.Foundation.PropertyType.String) }, // UserComment

            // Layer 2: Standard EXIF fields
            { "/app1/ifd/{ushort=305}", new BitmapTypedValue($"ManpLab v{metadata.Version}", Windows.Foundation.PropertyType.String) }, // Software
            { "/app1/ifd/{ushort=270}", new BitmapTypedValue(description, Windows.Foundation.PropertyType.String) }, // ImageDescription
            { "/app1/ifd/{ushort=315}", new BitmapTypedValue("ManpLab", Windows.Foundation.PropertyType.String) }, // Artist
            { "/app1/ifd/{ushort=33432}", new BitmapTypedValue("https://github.com/markhassellsmith/ManpLab", Windows.Foundation.PropertyType.String) }, // Copyright

            // Layer 3: DateTime fields (EXIF DateTimeOriginal and DateTimeDigitized)
            { "/app1/ifd/exif/{ushort=36867}", new BitmapTypedValue(metadata.RenderDate.ToString("yyyy:MM:dd HH:mm:ss"), Windows.Foundation.PropertyType.String) }, // DateTimeOriginal
            { "/app1/ifd/exif/{ushort=36868}", new BitmapTypedValue(metadata.RenderDate.ToString("yyyy:MM:dd HH:mm:ss"), Windows.Foundation.PropertyType.String) }  // DateTimeDigitized
        };
