/*
    DDFRACTINTFUNCTIONS.CPP a module for the per pixel calculations of fractals originally from Fractint 
...
int	CPixel::DDInitFractintFunctions(WORD type, DDComplex *z, DDComplex *q)
    {
    switch (type)
	{
	case NEWTON:
	...
	case MPNEWTON:
		...
		*degree = 3;				// defaults to 3, but 2 is possible 
		...
		for (int i = 0; i < *degree; i++)
		    {
		    rootsDD[i].x = cos(i*PI*2.0 / (double)*degree);
		    rootsDD[i].y = sin(i*PI*2.0 / (double)*degree);
		    }
		}
	...
	case MANOWAR:					// to handle fractint par files
		...
		*z = tDD;
		...
		{
		z->x = tempDD.x = tDD.x + param[0];
		z->y = tempDD.y = tDD.y + param[1];
		}
	    sqrDD.x = sqr(z->x);
	    sqrDD.y = sqr(z->y);
	    temp1DD = *z;
...
//	    t = (invert) ? invertz2(*c) : *c;
	...
	case COMPLEXBASIN:
	    ...
	    cdegreeDD.x = param[0];
	    cdegreeDD.y = param[1];
//	    thresh = 0.3 * PI / cdegreeDD.x;		// less than half distance between roots
	    *z = *q;
	...
	case PHOENIXFPCPLX:
	    ...
	    temp3DD = 0.0;				// set temp3 to Y value
	    ...
	    if (PhoenixDegree == 0)
		{
		if (temp2DD.x != 0.0 || temp2.y != 0.0)
		    *symmetry = NOSYM;
		else
		    *symmetry = ORIGIN;
		if (tempDD.y == 0.0 && temp2DD.y == 0.0)
		    *symmetry = XAXIS;
		PhoenixType = ZERO;
		}
	    if (PhoenixDegree >= 2)
	...
	case TETRATEFP:					// Tetrate(XAXIS) { c=z=pixel: z=c^z, |z|<=(P1+3)
	    ...
	    sqrDD.x = sqr(z->x);
	    sqrDD.y = sqr(z->y);
	    temp1DD = *z;					// set temp3 to Y value
	    break;
	...
	case MAGNET1M:
	case MAGNET1J:
	    DDInitTierazonFunctions(102, z, q);
	    break;

	case MAGNET2M:
	case MAGNET2J:
	    DDInitTierazonFunctions(103, z, q);
	    break;
...
int	CPixel::DDRunFractintFunctions(WORD type, DDComplex *z, DDComplex *q, BYTE *SpecialFlag, long *iteration)
    {
    DDComplex   a, b, t, z1, z2, zd, zt;
	...
	case MANOWAR:						// to handle fractint par files
	    sqrDD.x = sqr(z->x);
	    sqrDD.y = sqr(z->y);
	    temp3DD.x = sqrDD.x - sqrDD.y + tempDD.x + temp1DD.x + param[0];
	    temp3DD.y = 2.0 * z->x * z->y + tempDD.y + temp1DD.y + param[1];
	    temp1DD = *z;
//	    d = z->CSumSqr();
	    *z = temp3DD;
	    return DDBailoutTest(z, sqrDD);
...
//		z->y = (foldyinitx + q->y + foldxinity + param[1]);
	...
	case COMPLEXBASIN:
		...
		temp = z->CLog();
		tmp = tempDD * cdegreeDD;
		mod = tmp.y / TWO_PI;
		modDble = to_double(mod);
		coloriter = (long)modDble;
	...
	case LAMBDAFP:
	case LAMBDA:
	    sqrDD.x = z->x - sqrDD.x + sqrDD.y;
	    sqrDD.y = -(z->y * z->x);
	    sqrDD.y += sqrDD.y + z->y;

	    z->x = tempDD.x * sqrDD.x - tempDD.y * sqrDD.y;
	...
	case QUATJULFP:
	    ...
	    dd_real a0, a1, a2, a3, n0, n1, n2, n3, magnitude;
	    a0 = z->x;
	    a1 = z->y;
	    a2 = tempDD.x;
	    a3 = tempDD.y;

	    n0 = a0 * a0 - a1 * a1 - a2 * a2 - a3 * a3 + qcDD;
	    n1 = 2 * a0*a1 + qciDD;
	    n2 = 2 * a0*a2 + qcjDD;
	    n3 = 2 * a0*a3 + qckDD;
	    // Check bailout
	...
	case MAGNET1M:
	case MAGNET1J:
	    return (DDRunTierazonFunctions(102, z, q, &z2, SpecialFlag, iteration));

	case MAGNET2M:
	case MAGNET2J:
	    return (DDRunTierazonFunctions(103, z, q, &z2, SpecialFlag, iteration));
