	...
	case MPNEWTON:
		...
		*degree = 3;				// defaults to 3, but 2 is possible 
		...
		for (int i = 0; i < *degree; i++)
		    {
		    rootsQD[i].x = cos(i*PI*2.0 / (double)*degree);
		    rootsQD[i].y = sin(i*PI*2.0 / (double)*degree);
		    }
		}
...
//	    thresh = 0.3 * PI / cdegreeDD.x;		// less than half distance between roots
	...
	case TETRATEFP:					// Tetrate(XAXIS) { c=z=pixel: z=c^z, |z|<=(P1+3)
	    ...
	    sqrQD.x = sqr(z->x);
	    sqrQD.y = sqr(z->y);
	    temp1QD = *z;					// set temp3 to Y value
	    break;
...
int	CPixel::QDRunFractintFunctions(WORD type, QDComplex *z, QDComplex *q, BYTE *SpecialFlag, long *iteration)
	...
	case MANOWAR:						// to handle fractint par files
	    sqrQD.x = sqr(z->x);
	    sqrQD.y = sqr(z->y);
	    temp3QD.x = sqrQD.x - sqrQD.y + tempQD.x + temp1QD.x + param[0];
	    temp3QD.y = 2.0 * z->x * z->y + tempQD.y + temp1QD.y + param[1];
	    temp1QD = *z;
//	    d = z->CSumSqr();
	    *z = temp3QD;
	    return QDBailoutTest(z, sqrQD);
...
//		z->y = (foldyinitx + q->y + foldxinity + param[1]);
	...
	case LAMBDAFP:
	case LAMBDA:
	    sqrQD.x = z->x - sqrQD.x + sqrQD.y;
	    sqrQD.y = -(z->y * z->x);
	    sqrQD.y += sqrQD.y + z->y;

	    z->x = tempQD.x * sqrQD.x - tempQD.y * sqrQD.y;
	...
	case QUATJULFP:
	    ...
	    qd_real a0, a1, a2, a3, n0, n1, n2, n3, magnitude;
	    a0 = z->x;
	    a1 = z->y;
	    a2 = tempDD.x;
	    a3 = tempDD.y;

	    n0 = a0 * a0 - a1 * a1 - a2 * a2 - a3 * a3 + qcQD;
	    n1 = 2 * a0*a1 + qciQD;
	    n2 = 2 * a0*a2 + qcjQD;
	    n3 = 2 * a0*a3 + qckQD;
	    // Check bailout
...
#undef D_TO_L
	...
	case RATIONALFRACTAL1:				// (z^2 + c) / (z^2 - c)
		{
		QDComplex z2 = z->CSqr();
		QDComplex numerator = z2 + *q;
		QDComplex denominator = z2 - *q;
	...
	case RATIONALFRACTAL2:				// (z^3 + c) / (z + c)
		{
		QDComplex z3 = z->CCube();
		QDComplex numerator = z3 + *q;
		QDComplex denominator = *z + *q;
		if (denominator.CSumSqr() < 1e-10)		// avoid division by zero
		*z = numerator * 1e10;
		else
		*z = numerator / denominator;
	...
	case CUBICFLYINGSQUIRREL:			// Cubic Flying Squirrel: z = z^3 + (c-1)*z - c
		{
		QDComplex z3 = z->CCube();
		QDComplex cMinus1 = *q - QDComplex(1.0, 0.0);
		*z = z3 + cMinus1 * *z - *q;
		return QDFractintBailoutTest(z);
		}
